...
//				parse2.result = this.head.applyBindings(parse2.bindings);
				...
				if (parse2.result.unify(term, true, bindings)) {
	...
	parseString(parse:NLParseRecord, context:NLContext, rule:NLPatternContainer, parser:NLParser, AI:RuleBasedAI, filterPartialParses:boolean) : NLParseRecord[]
	{
		...
		for(let nextToken of parse.nextTokens) {
			if (nextToken.token == null) {
				let parses2:NLParseRecord[] = this.parseString(new NLParseRecord(nextToken.next, parse.bindings, parse.ruleNames, parse.priorities), context, rule, parser, AI, filterPartialParses);
				if (parses2 != null) parses = parses.concat(parses2);
			} else if (nextToken.token == this.string) {
				// match!
				let parses2:NLParseRecord[] = this.destinationState.parse(new NLParseRecord(nextToken.next, parse.bindings, parse.ruleNames, parse.priorities), context, rule, parser, AI, filterPartialParses);
				if (parses2 != null) parses = parses.concat(parses2);
			}
		}
...
//				console.log("Matching POS, before: " + this.term.toString() + "\n  bindings: " + parse.bindings + "\n  Matching POS, after: " + term2.toString());
		for(let nextToken of parse.nextTokens) {
			if (nextToken.token == null) {
				let parses2:NLParseRecord[] = this.parsePOS(new NLParseRecord(nextToken.next, parse.bindings, parse.ruleNames, parse.priorities), context, rule, parser, AI, filterPartialParses);
				if (parses2 != null) parses = parses.concat(parses2);
			} else {
//						console.log("Matching POS "+term2.toString()+" with: " + nextToken.token);
				for(let POS of nextToken.POS) {
					...
					if (POS.term.unify(term2, true, bindings)) {
						let parses2:NLParseRecord[] = this.destinationState.parse(new NLParseRecord(nextToken.next, parse.bindings.concat(bindings), parse.ruleNames, parse.priorities), context, rule, parser, AI, filterPartialParses);
						if (parses2 != null) parses = parses.concat(parses2);
					}
				}
			}
...
//			console.log("CompiledNLPatternTransition.parsePattern: Using the raw rules for " + term2.functor.name + " ...");
	...
	//						console.log("  considering rule with head: " + rule2.head.toString() + "\n  new bindings: " + bindings);
					let results:NLParseRecord[] = rule2.parseWithBindings(new NLParseRecord(parse.nextTokens, bindings, parse.ruleNames, parse.priorities), false, context, parser, AI);
...
//				console.log("Result of pattern: " + parses_p);
	...
	//			console.log("parseFunction, pr.bindings: " + pr.bindings);
				let parses2:NLParseRecord[] = this.destinationState.parse(new NLParseRecord(pr.nextTokens, pr.bindings, pr.ruleNames, pr.priorities), context, rule, parser, AI, filterPartialParses);
				if (parses2 != null) parses_p = parses_p.concat(parses2);
			}
		}
		return parses_p;
