class Memorize_IntentionAction extends IntentionAction {
	...
	execute(ir:IntentionRecord, ai:RuleBasedAI) : boolean
	{
		...
		let negated_s_l:Sentence[] = [];
		if (s_l.length == 1) {
			if (s_l[0].getAllVariables().length > 0) variablesPresent = true;
			negated_s_l = s_l[0].negate();
			if (negated_s_l == null) {
				console.error("executeIntention memorize: cannot negate sentence!: " + s_l[0]);		
				return true;
			}
		} else {
			let negated_s:Sentence = new Sentence([],[]);
		...
		console.log("executeIntention memorize: negated_s_l = " + negated_s_l);
		...
		if (variablesPresent) {
			...
			ai.inferenceProcesses.push(new InferenceRecord(ai, [], [negated_s_l], 1, 0, false, null, new Memorize_InferenceEffect(intention), ai.o));
		} else {
			ai.inferenceProcesses.push(new InferenceRecord(ai, [], [s_l,negated_s_l], 1, 0, false, null, new Memorize_InferenceEffect(intention), ai.o));				
		}
		*/
		return true;
	}
