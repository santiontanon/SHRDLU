class BWAnswerWhere_IntentionAction extends IntentionAction {
...
//		if (intention.functor == ai.o.getSort("action.answer.whereto")) where_answer = INFERENCE_RECORD_EFFECT_ANSWER_WHERETO;
		...
		if (intention.attributes[2] instanceof ConstantTermAttribute &&
			...
			let target1:Sentence[] = [new Sentence([new Term(ai.o.getSort("space.directly.on.top.of"),
															[intention.attributes[2],
															 new VariableTermAttribute(ai.o.getSort("#id"), "WHERE")])],[false])];
			let target2:Sentence[] = [new Sentence([new Term(ai.o.getSort("space.inside.of"),
															[intention.attributes[2],
															 new VariableTermAttribute(ai.o.getSort("#id"), "WHERE")])],[false])];
			let target3:Sentence[] = [new Sentence([new Term(ai.o.getSort("space.at"),
															[intention.attributes[2],
															 new VariableTermAttribute(ai.o.getSort("#id"), "WHERE")])],[false])];
			ai.queuedInferenceProcesses.push(new InferenceRecord(ai, [], [target1, target2, target3], 1, 0, false, null, new BWAnswerWhere_InferenceEffect(intention, intention.functor == ai.o.getSort("action.answer.whereto"))));
...
//				console.log("target2: " + target2);
			ai.queuedInferenceProcesses.push(new InferenceRecord(ai, [], [target1,target2], 1, 0, false, null, new BWAnswerWhere_InferenceEffect(intention, intention.functor == ai.o.getSort("action.answer.whereto"))));
			...
			let target2:Sentence[] = [new Sentence([new Term(ai.o.getSort("space.at"),
															[intention.attributes[1],
															 new VariableTermAttribute(ai.o.getSort("#id"), "WHERE")])],[false])];
//				console.log("target2: " + target2);
			ai.queuedInferenceProcesses.push(new InferenceRecord(ai, additionalTerms, [target1,target2], 1, 0, false, null, new BWAnswerWhere_InferenceEffect(intention, intention.functor == ai.o.getSort("action.answer.whereto"))));
		} else {
	...
	{
		return "<IntentionAction type=\"BWAnswerWhere_IntentionAction\"/>";
	}


	static loadFromXML(xml:Element, ai:RuleBasedAI) : IntentionAction
	{
		return new BWAnswerWhere_IntentionAction();
	}
}
